...
const ExportPage = () => {
  ...
  const handleDeleteExport = async (e, id, code) => { e.stopPropagation(); if (window.confirm(`Xóa phiếu ${code}?`)) { try { await axiosClient.delete(`/exports/${id}`); toast.success('Đã xóa'); fetchData(); } catch (error) { toast.error('Lỗi xóa: ' + error.message); } } };
