...
const ExportPage = () => {
  ...
  const safeExports = Array.isArray(exports) ? exports : (exports?.data || []);

  const INITIAL_EXPORT_STATE = { code: '', customer_id: '', note: '', payment_due_date: '', hide_price: false, apply_wholesale: false, details: [] };
  ...
  const fetchNewCode = async () => { try { const res = await axiosClient.get('/exports/new-code'); setNewExport(prev => ({ ...prev, code: res.code })); } catch (error) { console.error(error); } };

  const DRAFT_KEY = 'export_draft_data';

  useEffect(() => {
    if (showModal && !isViewMode) {
       if (newExport.details.length > 0) { localStorage.setItem(DRAFT_KEY, JSON.stringify({ details: newExport.details })); } 
      ...
      if (savedDraft) {
         try {
             ...
             if (parsedDraft.details && parsedDraft.details.length > 0) {
                 setNewExport(prev => ({ ...INITIAL_EXPORT_STATE, code: prev.code, details: parsedDraft.details }));
                 setSelectedCustomerInfo(null);
                 toast.info('Đã khôi phục danh sách sản phẩm cũ', { autoClose: 2000 });
             }
         } catch (e) { localStorage.removeItem(DRAFT_KEY); }
      } else {
          setNewExport(prev => ({ ...INITIAL_EXPORT_STATE, code: prev.code })); setSelectedCustomerInfo(null);
      }
    }
  ...
  const handleResetForm = () => {
    if (window.confirm('Bạn có chắc muốn xóa hết sản phẩm để tạo phiếu mới?')) {
        setNewExport(INITIAL_EXPORT_STATE); setSelectedCustomerInfo(null);
        setIdempotencyKey(uuidv4()); fetchNewCode(); toast.info('Đã làm mới form');
    }
  };
  ...
  useEffect(() => {
    const loadData = async () => { if (!globalCache.exports || refreshFlags.exports) { try { setLoading(true); const res = await axiosClient.get('/exports'); setExports(res?.data || res || []); updateCache('exports', res); } catch (error) { console.error(error); } finally { setLoading(false); } } };
    loadData();
  ...
  const fetchData = async () => { 
      try { 
          setLoading(true); 
          const [exportRes, customerRes, productRes] = await Promise.all([axiosClient.get('/exports'), axiosClient.get('/partners?type=customer'), axiosClient.get('/products')]); 
          setExports(exportRes?.data || exportRes || []); updateCache('exports', exportRes); 
  ...
  const generateInvoiceHTML = (item, hidePrice, isImageExport = false) => {
    const customerName = item.customer_id?.name || 'Khách lẻ'; const customerPhone = item.customer_id?.phone || ''; const customerAddress = item.customer_id?.address || ''; const saleName = 'Phan Thành Tiến - 0387.645.618'; const note = item.note || '';
    const totalPointsThisBill = item.details.reduce((sum, d) => sum + (d.quantity * (d.gift_points || 0)), 0);
    let finalPoints = 0; let startPoints = 0;
    if (item.partner_points_snapshot !== undefined && item.partner_points_snapshot !== null) { finalPoints = item.partner_points_snapshot; startPoints = finalPoints - totalPointsThisBill; } else { startPoints = item.customer_id?.saved_points || 0; finalPoints = startPoints + totalPointsThisBill; }
  ...
  const checkExportAction = (e, item, type) => { e.stopPropagation(); if (type === 'excel' || type === 'image') { performExport(item, type, false); return; } const customerId = item.customer_id?._id; const foundCustomer = safeCustomers.find(c => c._id === customerId); const customerHideSetting = foundCustomer ? foundCustomer.hide_price : false; performExport(item, type, customerHideSetting); };
  ...
  const exportExcel = (item, hidePrice) => {
    const customerName = item.customer_id?.name || 'Khách lẻ'; const customerPhone = item.customer_id?.phone || ''; const customerAddress = item.customer_id?.address || ''; const saleName = 'Phan Thành Tiến - 0387.645.618'; const createdDate = new Date(item.date); const dateStr = `Ngày ${createdDate.getDate()} tháng ${createdDate.getMonth() + 1} năm ${createdDate.getFullYear()}`;
    const totalQty = item.details.reduce((s, i) => s + i.quantity, 0); const totalRawPrice = item.details.reduce((s, i) => s + (i.quantity * i.export_price), 0); const totalPoints = item.details.reduce((s, i) => s + (i.quantity * (i.gift_points || 0)), 0); const oldPoints = (item.partner_points_snapshot !== undefined && item.partner_points_snapshot !== null) ? item.partner_points_snapshot - totalPoints : (item.customer_id?.saved_points || 0) - totalPoints;
    const styleBorder = { top: { style: "thin" }, bottom: { style: "thin" }, left: { style: "thin" }, right: { style: "thin" } }; const styleHeaderTable = { font: { bold: true }, alignment: { horizontal: "center", vertical: "center", wrapText: true }, border: styleBorder, fill: { fgColor: { rgb: "EFEFEF" } } }; const styleCellBorder = { border: styleBorder };
    const ws_data = [ ["NPP LÂM ANH", "", "", "", "", "", "", "Số phiếu:", item.code], ["Mã số thuế: 1801790506"], ["Địa chỉ: 430B, KV1, P. Cái Răng, TP. Cần Thơ"], [], ["PHIẾU GIAO HÀNG"], [dateStr], [], ["Khách hàng:", customerName, "", "", "", "", "Điện thoại:", customerPhone], ["Địa chỉ:", customerAddress, "", "", "", "", "Sale:", saleName], ["Ghi chú:", item.note || ''], [] ];
    const tableHeaderRowIndex = 11;
    let tableHead = ["STT", "Tên sản phẩm", "ĐVT"]; if (!hidePrice) { tableHead.push("Đơn giá", "SL", "Tiền hàng", "CK(%)", "Thành tiền"); } else { tableHead.push("SL"); } tableHead.push("Điểm quà"); ws_data.push(tableHead);
    item.details.forEach((d, index) => { let row = [ index + 1, d.product_name_backup, d.unit ]; if (!hidePrice) { row.push(d.export_price, d.quantity, d.export_price * d.quantity, d.discount ? d.discount + '%' : '', d.total); } else { row.push(d.quantity); } row.push(d.quantity * (d.gift_points || 0)); ws_data.push(row); });
    let totalRow = ["", "TỔNG CỘNG", ""]; if (!hidePrice) { totalRow.push("", totalQty, totalRawPrice, "", item.total_amount); } else { totalRow.push(totalQty); } totalRow.push(totalPoints); ws_data.push(totalRow);
    ws_data.push([]);
    let footerInfo = [`Điểm còn gửi: ${oldPoints} + ${totalPoints} = ${oldPoints + totalPoints}`];
    if(!hidePrice) { const spacer = ["", "", "", "", "", "TỔNG THANH TOÁN:", item.total_amount]; footerInfo = footerInfo.concat(spacer.slice(1)); ws_data.push([`Điểm còn gửi: ${oldPoints} + ${totalPoints} = ${oldPoints + totalPoints}`, "", "", "", "", "", "TỔNG THANH TOÁN:", item.total_amount]); } else { ws_data.push(footerInfo); }
    ws_data.push([]); ws_data.push(["", "Người lập phiếu", "", "", "Người giao hàng", "", "", "Người nhận hàng"]);
    const ws = XLSX.utils.aoa_to_sheet(ws_data);
    const range = XLSX.utils.decode_range(ws['!ref']);
    for (let R = range.s.r; R <= range.e.r; ++R) { for (let C = range.s.c; C <= range.e.c; ++C) { const cell_address = XLSX.utils.encode_cell({ r: R, c: C }); if (!ws[cell_address]) continue; if (R === 4) { ws[cell_address].s = { font: { bold: true, sz: 16 }, alignment: { horizontal: "center" } }; } else if (R === 5) { ws[cell_address].s = { font: { italic: true }, alignment: { horizontal: "center" } }; } else if (R === tableHeaderRowIndex) { ws[cell_address].s = styleHeaderTable; } else if (R > tableHeaderRowIndex && R <= tableHeaderRowIndex + item.details.length + 1) { const isTotalRow = R === (tableHeaderRowIndex + item.details.length + 1); let cellStyle = { ...styleCellBorder }; if (isTotalRow) { cellStyle.font = { bold: true }; } if (C === 0 || C === 2) { cellStyle.alignment = { horizontal: "center" }; } else if (C === 1) { cellStyle.alignment = { horizontal: "left" }; } else { cellStyle.alignment = { horizontal: "right" }; } ws[cell_address].s = cellStyle; } else if (R === tableHeaderRowIndex + item.details.length + 3) { if(C >= 6) ws[cell_address].s = { font: { bold: true, sz: 12 }, alignment: { horizontal: "right" } }; } } }
    const wscols = !hidePrice ? [ { wch: 5 }, { wch: 30 }, { wch: 8 }, { wch: 10 }, { wch: 6 }, { wch: 12 }, { wch: 6 }, { wch: 12 }, { wch: 8 } ] : [ { wch: 5 }, { wch: 40 }, { wch: 10 }, { wch: 10 }, { wch: 10 } ]; ws['!cols'] = wscols;
    const lastCol = !hidePrice ? 8 : 4; 
    ws['!merges'] = [ { s: { r: 0, c: 0 }, e: { r: 0, c: 4 } }, { s: { r: 1, c: 0 }, e: { r: 1, c: 4 } }, { s: { r: 2, c: 0 }, e: { r: 2, c: 4 } }, { s: { r: 0, c: 7 }, e: { r: 0, c: 7 } }, { s: { r: 0, c: 8 }, e: { r: 0, c: lastCol } }, { s: { r: 4, c: 0 }, e: { r: 4, c: lastCol } }, { s: { r: 5, c: 0 }, e: { r: 5, c: lastCol } }, { s: { r: 7, c: 1 }, e: { r: 7, c: 5 } }, { s: { r: 7, c: 7 }, e: { r: 7, c: lastCol } }, { s: { r: 8, c: 1 }, e: { r: 8, c: 5 } }, { s: { r: 8, c: 7 }, e: { r: 8, c: lastCol } }, { s: { r: 9, c: 1 }, e: { r: 9, c: lastCol } }, ];
    const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "PhieuXuat"); XLSX.writeFile(wb, `Phieu_${item.code}.xlsx`);
  };

  const getProcessedExports = () => { let result = [...safeExports]; if (searchTerm) { const lowerTerm = searchTerm.toLowerCase().trim(); result = result.filter(item => { const matchCode = item.code?.toLowerCase().includes(lowerTerm); const matchCustomer = item.customer_id?.name?.toLowerCase().includes(lowerTerm); const matchNote = item.note?.toLowerCase().includes(lowerTerm); const dateStr = new Date(item.date).toLocaleDateString('vi-VN'); const matchDate = dateStr.includes(lowerTerm); const matchProduct = item.details?.some(d => d.product_name_backup?.toLowerCase().includes(lowerTerm) || d.sku?.toLowerCase().includes(lowerTerm)); return matchCode || matchCustomer || matchNote || matchDate || matchProduct; }); } return result.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)); };
  ...
  const handleSaveExport = async () => {
    ...
    try { 
        ...
        const payload = { ...newExport, total_amount: calculateTotalAmount(), idempotency_key: idempotencyKey };
  ...
  const handleRowClick = (item) => { 
      setNewExport({ _id: item._id, code: item.code, customer_id: item.customer_id?._id || '', note: item.note || '', payment_due_date: toInputDate(item.payment_due_date), hide_price: item.hide_price || false, details: item.details || [], date: item.date, total_amount: item.total_amount, partner_points_snapshot: item.partner_points_snapshot }); 
      const oldCustomer = safeCustomers.find(c => c._id === (item.customer_id?._id || item.customer_id)); 
      setSelectedCustomerInfo(oldCustomer); 
      setIsViewMode(true); 
      setShowModal(true); 
  };
  
  const handleCustomerChange = (customerId) => { 
      const customer = safeCustomers.find(c => c._id === customerId); 
      if (customer) { setSelectedCustomerInfo(customer); setNewExport(prev => ({ ...prev, customer_id: customerId, hide_price: customer.hide_price || false, apply_wholesale: customer.is_wholesale || false })); recalculatePrices(customer.is_wholesale || false); } else { setSelectedCustomerInfo(null); setNewExport(prev => ({ ...prev, customer_id: '', apply_wholesale: false })); } 
  };
  ...
  const recalculatePrices = (isWholesale) => { 
    setNewExport(prev => { 
        const newDetails = prev.details.map(item => { 
            ...
            return { ...item, export_price: originalPrice, discount: newDiscount, total: calculateLineTotal(item.quantity, originalPrice, newDiscount) }; 
        }); 
        return { ...prev, details: newDetails }; 
    }); 
  };
  
  const handleDownloadTemplate = () => { const templateData = [['Tên sản phẩm', 'Số lượng'], ['Sữa Ông Thọ', 10], ['Bánh Mì', 5]]; const ws = XLSX.utils.aoa_to_sheet(templateData); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "MauXuatKho"); XLSX.writeFile(wb, "Mau_Xuat_Kho.xlsx"); };
    ...
    reader.onload = (evt) => {
      try {
        ...
        for (let i = 1; i < data.length; i++) {
          ...
          if (product) {
            if (product.current_stock <= 0) { notFoundProducts.push(`${productName} (Hết hàng)`); continue; }
            const originalPrice = product.export_price || 0;
            let autoDiscount = 0; if (newExport.apply_wholesale) { autoDiscount = product.discount_percent || 0; }
            const finalQuantity = Math.min(quantity, product.current_stock); const qty = finalQuantity > 0 ? finalQuantity : 1;
            importedDetails.push({ product_id: product._id, product_name_backup: product.name, sku: product.sku, unit: product.unit, quantity: finalQuantity > 0 ? finalQuantity : 1, export_price: originalPrice, discount: autoDiscount, gift_points: product.gift_points || 0, total: calculateLineTotal(qty, originalPrice, autoDiscount) });
          } else { notFoundProducts.push(productName); }
        }
        if (importedDetails.length > 0) { setNewExport(prev => ({ ...prev, details: [...prev.details, ...importedDetails] })); toast.success(`Đã thêm ${importedDetails.length} sản phẩm từ file!`); }
        if (notFoundProducts.length > 0) { toast.warn(`Không thêm được ${notFoundProducts.length} SP: ${notFoundProducts.slice(0, 3).join(', ')}...`); }
  ...
  const addProductToExport = (product) => { 
    ...
    const originalPrice = product.export_price || 0; 
    let autoDiscount = 0; if (newExport.apply_wholesale) { autoDiscount = product.discount_percent || 0; }
    const newItem = { product_id: product._id, product_name_backup: product.name, sku: product.sku, unit: product.unit, quantity: 1, export_price: originalPrice, gift_points: product.gift_points || 0, discount: autoDiscount, total: calculateLineTotal(1, originalPrice, autoDiscount) }; 
    setNewExport({ ...newExport, details: [...newExport.details, newItem] }); setProductSearch(''); setFilteredProducts([]); setIsSearchFocus(true); setActiveIndex(-1); 
  };
  ...
  const updateDetail = (index, field, value) => {
    ...
    if (field === 'quantity') {
      const product = safeProducts.find(p => p._id === updatedDetails[index].product_id);
      if (!isViewMode && product && val !== '' && val > product.current_stock) {
        ...
        setNewExport({ ...newExport, details: updatedDetails }); return; 
      }
    }
  ...
  const handleBlur = (index, field) => {
    ...
    if (item[field] === '') {
        const updatedDetails = [...newExport.details]; updatedDetails[index][field] = 0;
        const qty = updatedDetails[index].quantity === '' ? 0 : updatedDetails[index].quantity; const price = updatedDetails[index].export_price === '' ? 0 : updatedDetails[index].export_price; const discount = updatedDetails[index].discount || 0;
        updatedDetails[index].total = calculateLineTotal(qty, price, discount); setNewExport({ ...newExport, details: updatedDetails });
    }
    if (field === 'export_price' && typeof item[field] === 'number') {
         const updatedDetails = [...newExport.details]; updatedDetails[index][field] = Math.round(item[field]);
         const qty = updatedDetails[index].quantity; const price = updatedDetails[index][field]; const discount = updatedDetails[index].discount || 0;
         updatedDetails[index].total = calculateLineTotal(qty, price, discount); setNewExport({ ...newExport, details: updatedDetails });
    }
  };
  ...
  const calculateLineTotal = (qty, price, discountPercent) => { const finalPrice = price * (1 - (discountPercent / 100)); return Math.round(qty * finalPrice); };
  ...
  useEffect(() => {
    ...
    if (isSearchFocus && productSearch.trim() === '') { results = safeProducts; } else {
      ...
      results = safeProducts.filter(p => p.name.toLowerCase().includes(lower) || (p.sku && p.sku.toLowerCase().includes(lower)) );
    }
  ...
  const handleKeyDown = (e) => {
    ...
    else if (e.key === 'Enter') { e.preventDefault(); if (activeIndex >= 0 && filteredProducts[activeIndex]) { addProductToExport(filteredProducts[activeIndex]); } else if (filteredProducts.length > 0) { addProductToExport(filteredProducts[0]); } } 
    ...
    <div className="p-2 pb-10">
      ...
      <div className="bg-white rounded-xl shadow-sm border overflow-hidden print:hidden">
            ...
            <tbody className="divide-y">
                ...
                <tr key={item._id} className="hover:bg-gray-100 cursor-pointer" onClick={() => handleRowClick(item)}>
                    ...
                    <td className="p-4 text-right font-bold text-gray-800">
                      ...
                      {(() => {
                          if (item.total_amount === 0) { return <div className="text-xs text-gray-400 mt-1 italic font-normal">---</div>; }
                          ...
                          if (!hasStoredProfit) {
                              const totalCost = item.details.reduce((sum, d) => {
                                    ...
                                    if (unitCost === undefined || unitCost === null) { const currentProduct = safeProducts.find(p => p._id === d.product_id); unitCost = currentProduct ? currentProduct.import_price : 0; }
                                    return sum + (Number(unitCost || 0) * d.quantity);
                              }, 0);
                              profit = item.total_amount - totalCost;
                          }
                          return ( <div className={`text-xs font-medium mt-1 ${profit >= 0 ? 'text-green-600' : 'text-red-500'}`}> {hasStoredProfit ? 'Lãi: ' : 'Lãi (ước tính): '} {formatCurrency(profit)}₫ </div> );
                      })()}
                  </td>
        ...
        {processedExports.length > 0 && (
            <div className="flex items-center justify-between px-4 py-3 border-t">
              <div className="text-sm text-gray-500"> Hiển thị {indexOfFirstItem + 1}-{Math.min(indexOfLastItem, processedExports.length)} trong số {processedExports.length} phiếu </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-2xl w-[98%] md:w-[90%] max-w-7xl h-[92vh] flex flex-col transform scale-100" style={{ animation: isClosing ? 'fadeOut 0.1s ease-out forwards' : 'fadeIn 0.1s ease-out forwards' }}>
             <div className="flex justify-between items-center p-5 border-b border-gray-200 shrink-0">
              <div className="flex items-center gap-4 overflow-hidden">
                  <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2 truncate">{isViewMode ? <><Eye size={24} className="text-blue-600 shrink-0" /> Chi tiết phiếu xuất</> : <><Plus size={24} className="text-blue-600 shrink-0" /> Tạo đơn bán hàng</>}</h2>
                  {!isViewMode && (
                    <button type="button" onClick={handleResetForm} className="flex items-center gap-1 text-sm bg-gray-100 hover:bg-gray-200 text-gray-600 px-3 py-1.5 rounded-full transition-colors border border-gray-200 whitespace-nowrap shrink-0" title="Xóa trắng form để nhập mới"><RefreshCw size={14} /> Làm mới</button>
                  )}
              </div>
              ...
              <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200 mb-5">
                <div className="grid grid-cols-4 gap-5 mb-4">
                    ...
                    <div>
                      ...
                      <Select options={customerOptions} value={customerOptions.find(c => c.value === newExport.customer_id) || null} onChange={(selected) => handleCustomerChange(selected ? selected.value : '')} isDisabled={isViewMode} placeholder="-- Nhập tên --" isClearable isSearchable noOptionsMessage={() => "Không tìm thấy tên này"} styles={{ control: (base) => ({ ...base, borderRadius: '0.5rem', borderColor: '#d1d5db', minHeight: '42px', fontSize: '14px' }), menu: (base) => ({ ...base, zIndex: 9999 }) }} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Hạn thanh toán</label>
                      <input type={isViewMode ? "text" : "date"} className="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-1 focus:ring-blue-500 outline-none text-gray-800" value={isViewMode ? formatDisplayDate(newExport.payment_due_date) : newExport.payment_due_date} onChange={(e) => setNewExport({...newExport, payment_due_date: e.target.value})} disabled={isViewMode} placeholder="dd/mm/yyyy" />
                    </div>
                    <div>
            ...
            <div className="p-5 border-t bg-white flex justify-end items-center rounded-b-xl shrink-0 gap-6">
              <div className="text-gray-600 font-medium flex items-center gap-2">
                  <DollarSign className="text-blue-600" /> Tổng tiền: <span className="text-2xl font-bold text-red-600 ml-1">{Math.round(calculateTotalAmount()).toLocaleString('vi-VN')}₫</span>
              </div>
              <div className="text-gray-600 font-medium flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                  ...
                  {(() => {
                      const totalPoints = calculateTotalPoints(); let startPoints = 0; let finalPoints = 0; let hasCustomer = false;
                      if (isViewMode && newExport.partner_points_snapshot !== undefined && newExport.partner_points_snapshot !== null) { hasCustomer = true; finalPoints = newExport.partner_points_snapshot; startPoints = finalPoints - totalPoints; } else if (selectedCustomerInfo) { hasCustomer = true; startPoints = selectedCustomerInfo.saved_points || 0; finalPoints = startPoints + totalPoints; }
                      if (hasCustomer) {
                          return ( <> <span className="text-sm">Điểm còn gửi:</span> <span className={`text-xl font-bold ${finalPoints < 0 ? 'text-red-600' : 'text-blue-600'}`}>{finalPoints}</span> <span className="text-sm text-gray-500 font-normal">({startPoints} {totalPoints >= 0 ? '+' : ''}{totalPoints})</span> </> );
                      } else {
                          return ( <> <span className="text-sm">Tổng điểm phiếu:</span> <span className={`text-xl font-bold ml-1 ${totalPoints >= 0 ? 'text-green-600' : 'text-red-600'}`}>{totalPoints > 0 ? '+' : ''}{totalPoints}</span> </> );
                      }
                  })()}
              </div>
              <div className="flex gap-3">
                <button onClick={() => { if (isViewMode && newExport._id) { handleUpdateNote(); } handleCloseModal(); }} className="px-6 py-2.5 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-100 font-medium transition-colors">Đóng</button>
                {!isViewMode && (
                  <button onClick={handleSaveExport} className={`px-6 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-bold flex items-center gap-2 shadow-lg transition-colors ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}>
